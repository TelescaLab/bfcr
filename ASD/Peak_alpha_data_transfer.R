...
# Demographic Data
demDat <- read.csv(file=paste0(path, '/demographic_data.csv'), header = TRUE)
colnames(demDat) <- c("ID", "Gender", "Age", "Group", "VIQ", "NVIQ")
demDat <- demDat[which(demDat$ID %in% subj_id), ]
signal_ratio <- readRDS(file=paste0(path, '/signal_ratio.rds'))
demDat <- cbind(demDat, signal_ratio)
colnames(demDat) <- c("ID", "Gender", "Age", "Group", "VIQ", "NVIQ", "Signal_ratio")

# Peak Alpha Data
load(file=paste0(path, '/pa.dat.Rdata'))
# ID: subject ID
# group: TD(1) or ASD (2)
...
pa.dat$Signal_ratio <- NA
for(i in 1:nrow(pa.dat)){
  pa.dat$Signal_ratio[i] <- demDat$Signal_ratio[demDat$ID == pa.dat$ID[i]] 
}
pa.dat %>%
  filter(reg == 15) %>%
