...
covtruth3 <- Btru%*%Lambda1%*%outer(x,x)%*%t(Lambda1)%*%t(Btru) + Btru%*%Lambda2%*%outer(x,x)%*%t(Lambda2)%*%t(Btru)

sum((covfreq - covtruth)^2)/sum((covtruth)^2)*100
sum((covbayes - covtruth)^2)/sum((covtruth^2))*100
par(mfrow = c(1,3))
persp3D(1:tmax,1:tmax, covtruth, theta=90,phi=10, main = "Truth", colkey = FALSE,zlim = c(min(unlist(covtruth)),max(unlist(covtruth))), zlab = "c(s,t,0)")
...
persp3D(1:tmax,1:tmax, covtruth1, theta=90,phi=10, main = "z = 0", colkey = FALSE,zlim = c(my_min,my_max), zlab = "", xlab = "t", ylab = "s")
persp3D(1:tmax,1:tmax, covtruth2, theta=90,phi=10, main = "z = .5", colkey = FALSE,zlim = c(my_min,my_max), zlab = "",xlab = "t", ylab = "s")
persp3D(1:tmax,1:tmax, covtruth3, theta=90,phi=10, main = "z = 1", colkey = FALSE,zlim = c(my_min,my_max), zlab = "",xlab = "t", ylab = "s")
...
temp <- numeric(tmax)
for(k in 1:K){
  temp <- temp + bayes_param$Eta[[1]][subj,k,iter] * B%*%bayes_param$Lambda[[1,iter]][,,k]%*%X[subj,]
}
lines(T, B%*%bayes_param$Theta[[1]][,,iter]%*%X[subj,] + temp, col = "red")
lines(T,B%*%bayes_param$Theta[[1]][,,iter]%*%X[subj,] + bayes_param$Eta[[1]][subj,1,iter] * B%*%bayes_param$Lambda[[1,iter]][,,1]%*%X[subj,]+
       bayes_param$Eta[[1]][subj,2,iter] * B%*%bayes_param$Lambda[[1,iter]][,,2]%*%X[subj,], col = "red")
#lines(B%*%bayes_param$Proj[[1]][subj,,iter],col="green")
lines(seq(from = 0, to = 1, length.out = tmax),B%*%rowMeans(bayes_param$Proj[[1]][subj,,burnin:max_iter]), col = "red")
sapply(burnin:max_iter, function(iter) lines(T,B%*%bayes_param$Proj[[1]][subj,,iter], col = "gray"))
sapply(burnin:max_iter, function(iter) lines(T,B%*%bayes_param$Theta[[1]][,,iter]%*%X[subj,] + bayes_param$Eta[[1]][subj,1,iter] * B%*%bayes_param$Lambda[[1,iter]][,,1]%*%X[subj,]+
