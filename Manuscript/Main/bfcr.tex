...
Overall, the simulation shows that the proposed method can lead to gains in estimation and coverage (compared to a homoschastic model) when the data has covariate-dependent heteroscedasticity. Cases 3 and 4 show that estimation error for the mean structure is robust to choice of model under a homoschedastic data-generating mechanism. However, making this adjustment under a homoschedastic data-generating mechanism leads to slightly larger estimation error in the covariance structure. We recommend exploring values of $g(t, x)$ to decide whether or not to adjust the variance by covariates. 
\begin{table}
	\caption{Simulation results over 300 data sets for each sample size and case combination. Coverage, error (RISE), and interval width are calculated as explained in Section \ref{s:simulation}}
	\label{t:simulation}
	\begin{center}
