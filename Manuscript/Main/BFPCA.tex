...
\end{gather*}
summarize the effect of $\bmath{x}_{r}$ across $\psi_{j}$, $j=1,\ldots,k$. Clearly $g_{r}(t, \bmath{x}_{r}) \in [0, 1]$ and $\sum_{r=1}^{R}g_{r}(t, \bmath{x}_{r}) = 1$. If the impact of $\bmath{x}_{r}$ on $\psi_{j}$, $j=1,\ldots,k$ is negligable, then $g_{r}(t, \bmath{x}_{r})$ will be near zero. Consequently if $g_{r}(t, \bmath{x}_{r})$ is near zero, the $c(t,t', \bmath{x})$ will not be sensitive to changes in $\bmath{x}_{r}$. To the best of our knowledge, this is the first attempt at quantifying the impact of covariates on a covariance function which is possible through the additivity assumption on $\psi_{j}(t, \bmath{x})$ in equation \ref{eq:psi_additivity}.
