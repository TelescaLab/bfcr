...
for (seed in 1:100) {
  ...
  Y <- time_basis[[1]]$X %*% beta %*% t(X) %>% t()
  for (kp in 1:k) {
    Y <- Y + time_basis[[1]]$X %*% lambda[,,kp] %*% t(X[1:n,]) %*% diag(eta[,kp]) %>% t()
  }
  Y_errors <- Y + rnorm(n * length(times), sd = .20) %>% matrix(n, length(times))
  mcmc_results <- run_mcmc(Y_errors, X_fit, X_fit, time_basis_fit[[1]]$X, times, penalties_mean, penalties_var, indices_mean, indices_var, k_fit, 10000, 5000, 1, "pooled")
  ...
  for (i in 1:10) {
    ...
    zi <- evaluate_basis(x_basis_fit[[1]], evaluated_points[i])
  ...
  file_name <- paste0("/Users/johnshamshoian/Documents/R_projects/bfcr/",
                      "Simulations/Metrics/", seed, ".RData")
